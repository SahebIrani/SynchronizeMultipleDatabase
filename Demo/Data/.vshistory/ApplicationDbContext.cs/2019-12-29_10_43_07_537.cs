...
namespace Demo.Data
{
	public class ApplicationDbContext : IdentityDbContext
	{
		...
		{
			var changedHere = await hereTable.Where(s => s.ChangedAt > from && s.ChangedAt <= to
													  || s.CreatedAt > from && s.CreatedAt <= to).ToListAsync();

			var changedThere = await thereTable.Where(s => s.ChangedAt > from && s.ChangedAt <= to
														|| s.CreatedAt > from && s.CreatedAt <= to
														|| s.SyncedAt > from && s.SyncedAt <= to).ToListAsync();

			var modifiedAtSameTimeOnBothSides = changedThere.Select(c => new { c.Id, c.ChangedAt })
															.Where(there => changedHere.Any(here => here.Id == there.Id && here.ChangedAt == there.ChangedAt))
															.Select(i => i.Id).ToList();

			changedHere = changedHere.Where(h => !modifiedAtSameTimeOnBothSides.Contains(h.Id)).ToList();
			changedThere = changedThere.Where(h => !modifiedAtSameTimeOnBothSides.Contains(h.Id)).ToList();

			var conflictIds = changedThere.Select(c => c.Id).Where(thereId => changedHere.Any(here => here.Id == thereId)).ToList();
		...
		private static void AddOrUpdateEntities<T>(IDbSet<T> destinationTable, IEnumerable<T> entityList, params string[] excludes)
